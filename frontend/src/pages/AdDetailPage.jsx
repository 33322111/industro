...
import notFavouriteIcon from "../assets/not_favourite.png";
...
const AdDetailPage = () => {
  ...
  useEffect(() => {
    const checkFavourite = async () => {
      try {
        const res = await api.get(`/favourites/check/?ad_id=${id}`);
    ...
    const fetchFavouriteCount = async () => {
      try {
        const res = await api.get(`/favourites/count/?ad_id=${id}`);
        setFavouriteCount(res.data.count);
      } catch (err) {
        console.error("Ошибка загрузки количества избранного:", err);
      }
    };
  ...
  const toggleFavourite = async () => {
    try {
      if (isFavourite) {
        await api.post("/favourites/remove/", { ad_id: id });
      } else {
        await api.post("/favourites/add/", { ad_id: id });
      }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg relative">
      ...
      <div className="mt-4 space-y-2">
        <p>
          <strong>Категория:</strong> {ad.category_name} / {ad.subcategory_name}
        </p>
        <p>
          <strong>Цена:</strong>{" "}
          {ad.price_type === "range"
        ...
        <p>
          <strong>Город:</strong> {ad.city || "Не указан"}
        </p>
        <p>
          <strong>Срок выполнения:</strong> {getExecutionTimeText()}
        </p>
        <p>
      ...
      {isMyOwnAd && (
        <div className="mt-6 flex flex-col gap-3">
          <p className="text-base text-gray-600">
            Добавлено в избранное: <strong>{favouriteCount}</strong> раз(а)
          </p>
